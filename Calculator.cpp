...
// 현재 결과창(Display) 문자열 반환
CString Calculator::GetDisplay() const { return m_buf; }

// 상단식/히스토리(RSHS) 문자열 반환
CString Calculator::GetRSHS() const { return m_bufo; }

// 커맨드 패턴 - 명령 종류에 따라 적절한 내부 함수 호출
void Calculator::Execute(const Command& cmd)
{
    switch (cmd.kind)
    {
    case CmdKind::Digit:        AppendDigit(cmd.digit); break;   // 숫자 입력
    case CmdKind::Decimal:      AppendDecimal(); break;          // 소수점 입력
    case CmdKind::Op:           SetOperator(cmd.op); break;      // 연산자 입력
    case CmdKind::Equal:        Equal(); break;                  // 등호
    case CmdKind::Clear:        Clear(); break;                  // 전체 초기화(C)
    case CmdKind::ClearEntry:   ClearEntry(); break;             // 입력만 초기화(CE)
    case CmdKind::Backspace:    Backspace(); break;              // 백스페이스
    case CmdKind::Percent:      Percent(); break;                // 퍼센트
    case CmdKind::ToggleSign:   ToggleSign(); break;             // 부호 전환(±)
    case CmdKind::Reciprocal:   Reciprocal(); break;             // 1/x
    case CmdKind::Square:       Square(); break;                 // x^2
    case CmdKind::SqrtX:        SqrtX(); break;                  // √x
    default: break;
    }
}
...
void Calculator::Clear()
{
    ...
    InvalidatePercentBase(); // '=' 직후 퍼센트 반복 기준 초기화
}

// 에러 메시지 설정 + 상태를 안전하게 정리
void Calculator::SetError(const CString& msg)
{
    ...
    InvalidatePercentBase(); // 에러 발생 시 퍼센트 반복 기준도 무효화
}

// 현재 m_buf 문자열을 실수(double)로 변환하여 반환
double Calculator::GetValue() const
{
    return _tstof(m_buf);
}

// 실수를 깔끔한 문자열로 포맷팅 (소수점 정리 등)
CString Calculator::FormatNumber(double v) const
{
    // 아주 작은 값은 0으로 스냅
    if (std::fabs(v) < 1e-15) v = 0.0;

    // 정수에 매우 가까우면 정수로 스냅 (표시 깔끔하게)
...
// m_buf에 숫자 값을 표시 문자열로 세팅
void Calculator::SetValue(double v)
{
    m_buf = FormatNumber(v);
}

// 숫자 문자열을 히스토리/표시용으로 정규화(0.00000 -> 0)
CString Calculator::NormalizeNumericText(const CString& rawText, bool keepTrailingDot) const
{
    ...
    // "2." 같은 입력은 표현 유지 옵션이 켜져 있으면 그대로 둠
    if (keepTrailingDot && t.Right(1) == _T(".")) return t;

    // 숫자 형태가 아니면(에러 문자열 등) 그대로 사용
    bool hasDigit = false;
...
bool Calculator::BeginNewEntryFromSpecial(const CString& newText)
{
    ...
    InvalidatePercentBase(); // 새 입력 시작이므로 '% 반복 기준'은 끊는다
...
void Calculator::Backspace()
{
    ...
    // [중요] 계산 결과값(루트, % 등)이 떠있는 상태에선 백스페이스가 안 먹히도록 함 (표준 계산기 동작)
    if (m_afterUnary || m_afterPercent) return;
...
void Calculator::ToggleSign()
{
    ...
    // 이항 연산자 입력 직후(m_newEntry==true)에 +/-를 누르면
    // RHS 입력이 시작된 것으로 간주해야 '='에서 rhs=left로 처리되지 않는다.
    ...
    // '=' 직후 +/-를 누르면 history의 숫자도 같이 갱신하되 '=' 표기는 유지
...
// 히스토리 문자열이 연산자 토큰으로 끝나는지 검사 (연산자 교체용)
...
// 현재 히스토리 끝에 rhs 텍스트를 붙여 마무리(= 포함 여부 선택)
...
void Calculator::SetOperator(Op op)
{
    ...
    // 0.00000 입력 후 + 눌렀을 때 표시창도 0으로 정규화
    NormalizeDisplayIfPossible();

    InvalidatePercentBase(); // 연산자 입력은 새로운 식을 시작하는 흐름이므로 '% 반복 기준'은 끊는다
...
void Calculator::Percent()
{
    ...
    if (m_pendingOp == Op::None)
    {
        if (History_IsJustEvaluated() || (m_afterPercent && m_percentBaseValid))
        {
            ...
            // base가 0이면 0%는 의미 없으니 0 유지 + 히스토리 어지럽히지 않음
...
// 단항 연산에서 사용할 피연산자(v)와 표기 문자열(argStr)을 준비
void Calculator::GetUnaryOperand(double& v, CString& argStr) const
{
    if (m_pendingOp != Op::None && m_newEntry)
    {
        v = m_acc;
        argStr = FormatNumber(m_acc); // 대기 상태(좌항 op)에서는 좌항을 대상으로
    }
    else
    {
        v = GetValue();
        argStr = NormalizeNumericText(m_buf, true); // 히스토리 표기는 항상 정규화
    }
}

// 단항 연산 결과를 히스토리에 표기(현재 식/연산자 상태에 맞춰 결합)
